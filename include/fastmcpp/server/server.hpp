...
namespace fastmcpp::server {
  ...
  public:
    ...
    explicit Server(std::string name = "fastmcpp_server",
        ...
        : name_(std::move(name)),
          ...
          website_url_(std::move(website_url)),
          icons_(std::move(icons)),
          strict_input_validation_(std::move(strict_input_validation)) {}

    // Route registration
    void route(const std::string& name, Handler h) { routes_[name] = std::move(h); }
    fastmcpp::Json handle(const std::string& name, const fastmcpp::Json& payload) const;

    // Middleware registration
    void add_before(BeforeHook h) { before_.push_back(std::move(h)); }
    void add_after(AfterHook h) { after_.push_back(std::move(h)); }

    // Metadata accessors (v2.13.0+)
    const std::string& name() const { return name_; }
    const std::string& version() const { return version_; }
    const std::optional<std::string>& website_url() const { return website_url_; }
    const std::optional<std::vector<fastmcpp::Icon>>& icons() const { return icons_; }
    const std::optional<bool>& strict_input_validation() const { return strict_input_validation_; }
