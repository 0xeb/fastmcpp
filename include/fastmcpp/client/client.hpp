...
class Client {
 public:
  Client() = default;
  explicit Client(std::unique_ptr<ITransport> t) : transport_(std::shared_ptr<ITransport>(std::move(t))) {}

  /// Set the transport (for deferred initialization)
  void set_transport(std::unique_ptr<ITransport> t) { transport_ = std::shared_ptr<ITransport>(std::move(t)); }

  /// Check if transport is connected
  bool is_connected() const { return transport_ != nullptr; }
  ...
  CallToolResult call_tool_mcp(
      const std::string& name,
      const fastmcpp::Json& arguments,
    ...
    auto invoke_request = [this, payload]() {
      return call("tools/call", payload);
    };
  ...
  CallToolResult call_tool(
      const std::string& name,
      const fastmcpp::Json& arguments,
      const std::optional<fastmcpp::Json>& meta = std::nullopt,
      std::chrono::milliseconds timeout = std::chrono::milliseconds{0},
      const std::function<void(float, std::optional<float>, const std::string&)>& progress_handler = nullptr,
  ...
  GetPromptResult get_prompt_mcp(
      const std::string& name,
  ...
  CompleteResult complete_mcp(
      const fastmcpp::Json& ref,
  ...
  Client new_() const { return new_client(); }

  /// Register roots/sampling/elicitation callbacks (placeholders for parity)
  void set_roots_callback(const std::function<fastmcpp::Json()>& cb) { roots_callback_ = cb; }
 ...
 private:
  ...
  fastmcpp::Json coerce_to_schema(const fastmcpp::Json& schema, const fastmcpp::Json& value) {
    ...
    if (type == "array") {
      ...
      const auto& items_schema = schema.contains("items") ? schema["items"] : fastmcpp::Json::object();
  ...
  CallToolResult parse_call_tool_result(const fastmcpp::Json& response, const std::string& tool_name) {
  ...
  ListResourceTemplatesResult parse_list_resource_templates_result(const fastmcpp::Json& response) {
    ...
    if (response.contains("resourceTemplates")) {
      for (const auto& r : response["resourceTemplates"]) {
        ...
        if (r.contains("description")) rt.description = r["description"].get<std::string>();
        if (r.contains("mimeType")) rt.mimeType = r["mimeType"].get<std::string>();
        if (r.contains("annotations")) rt.annotations = r["annotations"];
  ...
  InitializeResult parse_initialize_result(const fastmcpp::Json& response) {
    ...
    if (response.contains("capabilities")) {
      ...
      if (caps.contains("experimental")) result.capabilities.experimental = caps["experimental"];
      if (caps.contains("logging")) result.capabilities.logging = caps["logging"];
      if (caps.contains("prompts")) result.capabilities.prompts = caps["prompts"];
      if (caps.contains("resources")) result.capabilities.resources = caps["resources"];
      if (caps.contains("tools")) result.capabilities.tools = caps["tools"];
    }
