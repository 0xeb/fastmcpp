...
void test_session_creation()
{
    ...
    ServerSession session("sess_123", [&](const Json& msg) {
        sent.push_back(msg);
    });
...
void test_send_request_and_response()
{
    ...
    std::vector<Json> sent;
    ServerSession session("sess_1", [&](const Json& msg) {
        sent.push_back(msg);
    });

    // Start request in background thread
    std::future<Json> result_future = std::async(std::launch::async, [&]() {
...
void test_request_timeout()
{
    ...
    ServerSession session("sess_1", [](const Json&) {
...
void test_client_error_response()
{
    ...
    std::future<Json> result_future = std::async(std::launch::async, [&]() {
        return session.send_request("test/method", {});
    });
...
void test_request_id_generation()
{
    ...
    for (int i = 0; i < 5; i++)
    {
        std::future<Json> f = std::async(std::launch::async, [&]() {
            return session.send_request("test", {});
        });
