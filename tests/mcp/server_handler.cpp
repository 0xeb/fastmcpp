...
int main() {
  ...
  // call
  Json call = {{"jsonrpc","2.0"},{"id",3},{"method","tools/call"},{"params", Json{{"name","generate_chart"},{"arguments", Json{{"title","Sales"}}}}}};
  ...
  s.route("resources/list", [](const Json&) {
    return Json{{"resources", Json::array({
      Json{{"uri","file:///readme.txt"},{"name","readme.txt"}}
    })}};
  });
  ...
  // prompts/get route
  s.route("prompts/get", [](const Json& in) {
    auto args = in.value("arguments", Json::object());
    std::string who = args.value("name", std::string("there"));
    return Json{{"description","demo"},{"messages", Json::array({
      Json{{"role","user"},{"content","Hi " + who}}
    })}};
  });
  Json get_prompt = {
    {"jsonrpc","2.0"},
