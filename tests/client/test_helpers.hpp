...
std::shared_ptr<server::Server> create_tool_server() {
    ...
    static std::vector<client::ToolInfo> registered_tools = {
        ...
        {"typed", "Nested typed result", Json{{"type", "object"}}, Json{
            {"type", "object"},
            {"properties", {
                {"items", Json{
                    {"type", "array"},
        ...
        ,
        {"typed_invalid", "Invalid typed result", Json{{"type", "object"}}, Json{
            {"type", "object"},
            {"properties", {
                {"items", Json{
                    {"type", "array"},
    ...
    srv->route("tools/call", [](const Json& in) {
        ...
        if (name == "add") {
            ...
            Json rows = Json::array({
                ...
                Json{{"id", 2}, {"name", "two"}, {"active", false}, {"timestamp", "2025-01-02T00:00:00Z"}}
            });
            return Json{
                {"content", Json::array({{{"type", "text"}, {"text", "typed"}}})},
            ...
            return Json{
                ...
                {"isError", false},
                {"progress", Json::array({
                    Json{{"progress", 0.25}, {"message", "quarter"}},
            ...
            return Json{
                ...
                {"notifications", Json::array({
                    Json{{"method", "sampling/request"}, {"params", Json{{"x", 9}}}},
                    Json{{"method", "elicitation/request"}, {"params", Json{{"prompt", "ping"}}}},
                    Json{{"method", "roots/list"}, {"params", Json::object()}}
                })}
            };
        } else if (name == "typed_invalid") {
            return Json{
                {"content", Json::array({{{"type", "text"}, {"text", "bad"}}})},
                {"structuredContent", Json{{"items", Json::array({Json::object()})}, {"mode", "fast"}}},
                {"isError", false}
            };
        }
        return Json{
...
std::shared_ptr<server::Server> create_protocol_server(ProtocolState& state) {
    ...
    srv->route("notifications/poll", [&state](const Json&) {
        ...
        return Json{{"notifications", Json::array({
            Json{{"method", "sampling/request"}, {"params", Json{{"x", 21}}}},
            Json{{"method", "elicitation/request"}, {"params", Json{{"prompt", "hello"}}}},
            Json{{"method", "roots/list"}, {"params", Json::object()}}
        })}};
    });
