...
void test_tool_schema_properties()
{
    ...
    Json output_schema = {{"type", "object"}, {"properties", {{"greeting", {{"type", "string"}}}}}};

    tools::Tool greet{"greet", input_schema, output_schema, [](const Json& in) -> Json
...
void test_tool_with_nested_arrays()
{
    ...
    tools::Tool matrix{"matrix", Json{{"type", "object"}}, Json{{"type", "array"}},
                       [](const Json& in) -> Json
                       {
...
void test_tool_chaining()
{
    ...
    tools::Tool double_it{"double", Json{{"type", "object"}}, Json{{"type", "number"}},
                          [](const Json& in) { return in["n"].get<int>() * 2; }};

    tools::Tool add_ten{"add_ten", Json{{"type", "object"}}, Json{{"type", "number"}},
...
void test_tool_with_null_handling()
{
    ...
    tools::Tool null_check{
        "null_check", Json{{"type", "object"}}, Json{{"type", "object"}}, [](const Json& in) -> Json
        { return Json{{"is_null", in["value"].is_null()}, {"type", in["value"].type_name()}}; }};
...
void test_tool_with_unicode()
{
    ...
    tools::Tool echo{"unicode_echo", Json{{"type", "object"}}, Json{{"type", "string"}},
...
void test_tool_with_empty_schema()
{
    ...
    tools::Tool minimal{"minimal", Json{{"type", "object"}}, Json{{"type", "string"}},
...
void test_tool_special_characters_in_name()
{
    ...
    tools::Tool underscore{"my_tool_name", Json{{"type", "object"}}, Json{{"type", "number"}},
                           [](const Json&) { return 1; }};

    tools::Tool dash{"my-tool-name", Json{{"type", "object"}}, Json{{"type", "number"}},
                     [](const Json&) { return 2; }};

    tools::Tool numeric{"tool123", Json{{"type", "object"}}, Json{{"type", "number"}},
                        [](const Json&) { return 3; }};
...
void test_tool_large_json_input()
{
    ...
    tools::Tool sum_array{"sum_array", Json{{"type", "object"}}, Json{{"type", "number"}},
                          [](const Json& in)
                          {
                              int sum = 0;
...
void test_tool_deeply_nested_objects()
{
    ...
    tools::Tool deep_get{"deep_get", Json{{"type", "object"}}, Json{{"type", "string"}},
                         [](const Json& in) { return in["a"]["b"]["c"]["d"]["value"]; }};
...
void test_tool_boolean_logic()
{
    ...
    tools::Tool logic{
        "logic", Json{{"type", "object"}}, Json{{"type", "object"}}, [](const Json& in) -> Json
        {
            bool a = in["a"].get<bool>();
            bool b = in["b"].get<bool>();
            return Json{{"and", a && b}, {"or", a || b}, {"xor", a != b}, {"not_a", !a}};
        }};
...
void test_tool_float_precision()
{
    ...
    tools::Tool precise{"precise", Json{{"type", "object"}}, Json{{"type", "number"}},
                        [](const Json& in)
                        {
...
void test_tool_empty_string_handling()
{
    ...
    tools::Tool str_len{"str_len", Json{{"type", "object"}}, Json{{"type", "integer"}},
                        [](const Json& in)
...
void test_tool_json_serialization_roundtrip()
{
    ...
    tools::Tool passthrough{"passthrough", Json{{"type", "object"}}, Json{{"type", "object"}},
                            [](const Json& in) { return in; }};

    tm.register_tool(passthrough);

    Json complex = {{"string", "hello"},
                    {"number", 42},
                    {"float", 3.14},
                    {"bool", true},
                    {"null", nullptr},
...
void test_tool_exception_types()
{
    ...
    tools::Tool throws_runtime{"throws_runtime", Json{{"type", "object"}}, Json{{"type", "string"}},
                               [](const Json&) -> Json { throw std::runtime_error("runtime"); }};

    tools::Tool throws_logic{"throws_logic", Json{{"type", "object"}}, Json{{"type", "string"}},
                             [](const Json&) -> Json { throw std::logic_error("logic"); }};

    tools::Tool throws_range{"throws_range", Json{{"type", "object"}}, Json{{"type", "string"}},
    ...
    try
    {
        tm.invoke("throws_runtime", Json{});
    }
    catch (const std::runtime_error& e)
    {
        caught = true;
        assert(std::string(e.what()) == "runtime");
    }
    assert(caught);
    ...
    try
    {
        tm.invoke("throws_logic", Json{});
    }
    catch (const std::logic_error& e)
    {
        caught = true;
        assert(std::string(e.what()) == "logic");
    }
    assert(caught);
    ...
    try
    {
        tm.invoke("throws_range", Json{});
    }
    catch (const std::out_of_range& e)
    {
        caught = true;
        assert(std::string(e.what()) == "range");
    }
    assert(caught);
...
void test_tool_stateful_lambda()
{
    ...
    int counter = 0;
    tools::Tool stateful{"counter", Json{{"type", "object"}}, Json{{"type", "integer"}},
...
void test_tool_closure_capture()
{
    ...
    tools::Tool prefixer{"prefixer", Json{{"type", "object"}}, Json{{"type", "string"}},
                         [prefix](const Json& in)
                         { return prefix + in["value"].get<std::string>(); }};
