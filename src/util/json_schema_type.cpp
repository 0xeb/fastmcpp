...
SchemaValue convert_union(const std::vector<fastmcpp::Json>& schemas,
                          const fastmcpp::Json& instance, const std::string& path)
{
    ...
    throw fastmcpp::ValidationError("No union branch matched at " + path +
                                    (last_error.empty() ? "" : (": " + last_error)));
}

void enforce_enum_const(const fastmcpp::Json& schema, const fastmcpp::Json& instance,
                        const std::string& path)
{
    ...
    if (schema.contains("enum"))
    {
        ...
        if (!ok)
            throw fastmcpp::ValidationError("Enum mismatch at " + path);
    }
}

SchemaValue handle_string(const fastmcpp::Json& schema, const fastmcpp::Json& instance,
                          const std::string& path)
{
    std::string value;
...
SchemaValue handle_number(const fastmcpp::Json& schema, const fastmcpp::Json& instance,
                          const std::string& path, bool integer)
{
    double num = 0.0;
    ...
    if (schema.contains("minimum"))
    {
        ...
        if (num < minv)
            throw fastmcpp::ValidationError("Value below minimum at " + path);
    }
    if (schema.contains("maximum"))
    {
        double maxv = schema["maximum"].get<double>();
        if (num > maxv)
            throw fastmcpp::ValidationError("Value above maximum at " + path);
    }
    if (schema.contains("exclusiveMinimum"))
    {
        double minv = schema["exclusiveMinimum"].get<double>();
        if (!(num > minv))
            throw fastmcpp::ValidationError("Value not greater than exclusiveMinimum at " + path);
    }
    if (schema.contains("exclusiveMaximum"))
    {
        double maxv = schema["exclusiveMaximum"].get<double>();
        if (!(num < maxv))
            throw fastmcpp::ValidationError("Value not less than exclusiveMaximum at " + path);
    }
...
SchemaValue handle_array(const fastmcpp::Json& schema, const fastmcpp::Json& instance,
                         const std::string& path)
{
    ...
    else
    {
        ...
        {
            out.push_back(
                convert(fastmcpp::Json::object(), instance[i], path + "/" + std::to_string(i)));
        }
    }
...
SchemaValue handle_object(const fastmcpp::Json& schema, const fastmcpp::Json& instance,
                          const std::string& path)
{
    ...
    if (schema.contains("properties") && schema["properties"].is_object())
    {
        for (const auto& [key, subschema] : schema["properties"].items())
        {
            ...
            else if (std::find(required.begin(), required.end(), key) != required.end())
            {
                throw fastmcpp::ValidationError("Missing required property: " + key + " at " +
                                                path);
            }
        }
    }
                    ...
                    const std::string& path)
{
    ...
    if (schema.contains("type") && schema["type"].is_string())
    {
        ...
        {
            if (!instance.is_null())
                throw fastmcpp::ValidationError("Expected null at " + path);
            return nullptr;
        }
        if (type == "boolean")
        {
            if (instance.is_boolean())
                return instance.get<bool>();
            if (instance.is_string())
                return instance.get<std::string>() == "true";
...
fastmcpp::Json schema_value_to_json(const SchemaValue& value)
{
    struct Visitor
    {
        fastmcpp::Json operator()(std::nullptr_t) const
        {
            return nullptr;
        }
        fastmcpp::Json operator()(bool b) const
        {
            return b;
        }
        fastmcpp::Json operator()(int64_t i) const
        {
            return i;
        }
        fastmcpp::Json operator()(double d) const
        {
            return d;
        }
        fastmcpp::Json operator()(const std::string& s) const
        {
            return s;
        }
        ...
        fastmcpp::Json operator()(const fastmcpp::Json& j) const
        {
            return j;
        }
    };
