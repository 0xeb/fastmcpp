...
Settings Settings::from_json(const Json& j) {
  Settings s;
  if (j.contains("log_level")) s.log_level = j.at("log_level").get<std::string>();
  if (j.contains("enable_rich_tracebacks")) s.enable_rich_tracebacks = j.at("enable_rich_tracebacks").get<bool>();
  return s;
}
