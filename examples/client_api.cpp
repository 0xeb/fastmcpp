...
std::shared_ptr<server::Server> create_sample_server()
{
                   ...
                   return Json{
                       {"tools",
                        Json::array({{{"name", "calculate"},
                                     ...
                                     {{"name", "echo"},
                                      {"description", "Echo back input with metadata"},
                                      {"inputSchema",
                                       {{"type", "object"},
    ...
    srv->route(
        ...
        [](const Json& in)
        {
            ...
            if (name == "calculate")
            {
                ...
                return Json{{"content",
                             Json::array({{{"type", "text"}, {"text", std::to_string(result)}}})},
                            {"isError", false}};
            }
            else if (name == "echo")
            {
                ...
                return Json{{"content",
                             Json::array({{{"type", "text"}, {"text", response_content.dump()}}})},
                            {"isError", false},
                            {"_meta", meta}};
            }
    ...
    srv->route("resources/list",
               [](const Json&)
               {
                   return Json{{"resources", Json::array({{{"uri", "config://app/settings"},
                                                           {"name", "App Settings"},
                 ...
                 Json::array(
                     {{{"name", "code_review"}, {"description", "Review code for best practices"}},
                      {{"name", "explain"},
                       ...
                       {"arguments", Json::array({{{"name", "topic"},
...
int main()
{
    ...
    auto calc_result = c.call_tool("calculate", {{"operation", "multiply"}, {"a", 7}, {"b", 6}});
    ...
    Json meta = {{"user_id", "user-123"}, {"trace_id", "trace-abc"}, {"tenant", "acme"}};
    ...
    auto opts_result =
        c.call_tool_mcp("calculate", {{"operation", "add"}, {"a", 100}, {"b", 200}}, opts);
